    ...
    loop {
        ...
        match message {
            ...
            | Message::Tip {
                ...
                match decide_tip_action(peer_height, peer_tip, local_height, local_tip) {
                    TipAction::RequestBlocks { start_height } => {
                        let msg = Message::GetBlocks { start_height };
                        let data = encode_message(&msg)?;
                        stream.write_all(&data).await?;
                    }
                    TipAction::RequestBlock { hash } => {
                        let msg = Message::GetBlock { hash };
                        let data = encode_message(&msg)?;
                        stream.write_all(&data).await?;
                    }
...
fn decide_tip_action(
    ...
    if peer_tip != local_tip {
        if peer_tip.0 > local_tip.0 {
            return TipAction::RequestBlock { hash: peer_tip };
        }
        return TipAction::SendTip {
            height: local_height,
            tip: local_tip,
...
mod tests {
    ...
    #[test]
    fn handles_equal_height_forked_tips() {
        let local_tip = Hash32([1u8; 32]);
        let peer_tip = Hash32([2u8; 32]);
        let action = decide_tip_action(5, peer_tip, 5, local_tip);
        assert_eq!(action, TipAction::RequestBlock { hash: peer_tip });

        let action = decide_tip_action(5, local_tip, 5, peer_tip);
        assert_eq!(
            action,
            TipAction::SendTip {
                height: 5,
