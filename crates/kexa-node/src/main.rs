...
async fn get_block(Path(hash): Path<String>, state: axum::extract::State<AppState>) -> Json<Block> {
    let guard = state.inner.lock().await;
    let bytes = hex::decode(hash).expect("hash");
    let mut h = [0u8; 32];
    h.copy_from_slice(&bytes);
    let block = guard.storage.get_block(&Hash32(h)).unwrap().expect("block");
    Json(block)
}
...
async fn get_balance(
    ...
    let address = Address::from_bech32(&address).expect("address");
    let guard = state.inner.lock().await;
    let utxos = guard
        .storage
...
fn init_genesis(storage: &Storage) -> Result<()> {
    ...
    let merkle = merkle_root(&[coinbase.clone()]);
